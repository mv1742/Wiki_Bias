...
sql_query_5 = "SELECT reverts, title, host FROM urls_article_title_all where title is not null ORDER BY reverts DESC LIMIT 5000;"
# Query result
dict_of_tables = {}
...
df_tables['Date received'] = pd.to_datetime(df_tables['Date received'])
...
left_column = [ html.H3(children='Select source/host & dataset size'),
               ...
               dcc.Slider(id="n-selection-slider",
                          ...
                          marks=make_n_marks(),
                          value=10),
               html.Label('Select a source, using either the dropdown or the plot below.',
                          style={'marginTop': 50}),
...
app.layout = html.Div(className='container', children=[
                html.H1(children='Reverts per source'),
...
def set_n(n_value, time_values):
    ...
    return {'data': [{'x': values_sample, 'y': counts_sample, 'type': 'bar', 'name': ''}],'layout': {'title': 'Top 20 sources'}}
