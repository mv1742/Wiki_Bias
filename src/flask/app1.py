...
sql_query_boxplot = "SELECT host FROM groupby_url_ratio_relevant LIMIT 10;"
df_boxplot = pd.read_sql_query(sql_query_boxplot, con)
dff = df_boxplot[:10]

sql_query_5 = "SELECT reverts, title, host FROM urls_article_title_all where title is not null ORDER BY reverts DESC LIMIT 20000;"
...
# def Add_Dash(server):
print('4/df_tables.ratio.max()', df_tables['Date received'].max())
...
def Add_Dash(server):
    ...
    left_column = dbc.Jumbotron(
        [
            ...
            html.Label("Select a host", style={"marginTop": 50}, className="lead"),
    ...
    body = dbc.Container(
        [
            ...
            dbc.Row([dbc.Col(wordcloud_plots)]),
            dbc.Row(
                [
    ...
    #     complaints_text = list(local_df["Host title"].dropna().values)
